...
export default class CSSUnparsedValue extends CSSStyleValue {
  constructor(members = []) {
    ...
    if (members.length < 1) {
      throw new TypeError(`Failed to construct 'CSSUnparsedValue': 1 argument required, but only ${members.length} present.`);
    }
  ...
  * [Symbol.iterator]() {
  ...
  set(index, val) {
    if (index < 0 || index >= this.length) {
      throw new RangeError(`Failed to set an indexed property on 'CSSUnparsedValue': The index provided (${index}) is outside the range [0, ${this.length - 1}].`);
    }
