...
export default class StylePropertyMapReadOnly {
  // Private, we only want direct access styles within the Class, not instances.
  #styles;
  ...
  constructor(key, styles) {
    ...
    for (let i = 0; i < styles.length; i++) {
      ...
      styleObj[propertyName] = CSSStyleValue.parse(propertyName, value);
    }
  ...
  *[Symbol.iterator]() {
   ...
   *
   * Note: We may want to change the behavior based on this from the spec
   * "In practice, since the values are "hidden" behind a .get() method call,
